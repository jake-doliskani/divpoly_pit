...
\ref{sec:exper} show, our algorithm attains better runtimes in practice.
...
\begin{algorithm}[Division polynomial computation]
	...
	\begin{algorithmic}[1]
		...
		\For {$i = k - s$ down to $0$}\label{step:divpoly-loop} 
...
Columns ``Alg. 2" and ``IsoGr" refer to Algorithm 2 of Section \ref{sec:pit-for-div} and the one in 
\cite{sutherland2012}, respectively. As the timings suggest, complexities of the two algorithms 
differ only by a constant factor, which confirms the theory. Also detecting ordinary curves is 
substantially faster on average than detecting supersingular curves, which again confirms the 
complexities claimed in Proposition \ref{prop:ss-strong}. 
