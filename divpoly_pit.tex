...
Let $\varmathbb{G}_m$ be the multiplicative group over $\F_q$. In \cite{voloch2010}, elements of 
high 
order in $\F_q^*$ are obtained using points on a curve contained in the fibered product $E \times 
\varmathbb{G}_m$. We can use the same technique to obtain points of high order on $E$. The proofs 
remain essentially the same except for some parts which we explain in the following. We fix an 
embedding $\varmathbb{G}_m \hookrightarrow \P^1$ and assume all curves are projective.
...
\begin{theorem}
	...
	Let $X \subset E \times \varmathbb{G}_m$ be an absolutely irreducible curve over $\F_q$ with 
...
\begin{proof}
	...
	$\F_q$-Frobenius map on $E \times \varmathbb{G}_m$. Then for $j \in J$ we have
	\[ \pi^j(P, b) = (\pi^j(P), \pi^j(b)) = (\pi^j(P), b^{q^j}) = (\pi^j(P), c^{n_j}) \]
	where $n_j$ corresponds to $j$, that is $n_j \le N$, $(n_j, q) = 1$, $n_j \bmod r \in 
	\Gamma$ and $n_j \equiv \gamma q^j \bmod r$. This means that there is a place of $K_{n_j}$ 
above 
...
curve $X$, e.g. an open subset of the graph of a morphism $f: E \rightarrow \varmathbb{P}^1$, we 
can 
obtain much better bounds on the order of $P$.  
\begin{theorem}
	\label{theo:order-ell}
	Let $E/\F_q$ be an elliptic curve and let $f: E \rightarrow \varmathbb{P}^1$ be the projection 
to 
...
\end{theorem}
\begin{proof}
	Let $X$ be an open subset of the graph of a morphism $f: E \rightarrow \varmathbb{P}^1$ defined 
by 
