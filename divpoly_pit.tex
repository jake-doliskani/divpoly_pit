...
order on $E$. For this, we follow the approach of Voloch \cite{voloch2007,voloch2010}.

Let $\mathbb{G}_m$ be the multiplicative group over $\F_q$. In \cite{voloch2010}, elements of high 
order in $\F_q^*$ are obtained using points on a curves contained in the fibered product $E \times 
\mathbb{G}_m$. We can use the same technique to obtain points of high order on $E$. The proofs 
remain essentially the same except for some minor details which explain in the following. We fix an 
...
has non-constant projections to both factors, and denote by $D$ the degree of $X \rightarrow 
\P^1$. Consider the commutative diagram
...
\begin{proof}
	...
	I}y_{n_j} - \sum_{j \in I'}y_{n_j}$ where the addition occurs on $E$ compared to $z = \prod_{j 
...
exponent $2T$ in the bound as shown in the proof. If $X$ is taken to be an open subset of the graph 
of a morphism $f: E \rightarrow \mathbb{P}^1$ then better bounds can be obtained. In fact, for our 
purposes it is enough to take $f$ as the projection to the first coordinate, i.e. $f(P) = x_P$. A 
point $(x_P, P)$ is in $X_n$ if $(x_P^n, P)$ is in $X$. Therefore, it is implied by the commutative 
digram
...
\begin{proof}
	...
	p)^2)$ operations in $\F_p$, \cite{shoup1994}. Step \ref{step:ss-psi} is performed at the cost 
	of $\tildO(\log^2 p)$ operations in $\F_p$ using Algorithm \ref{alg:divpoly-comp}. Therefore, 
	the total cost of the Algorithm \ref{alg:ss-strong} is $\tildO(\log^2 p)$ operations in $\F_p$.
\end{proof}
...
We have implemented Algorithm \ref{alg:ss-strong} in C++. The arithmetic in $\F_{p^2}[x]$ is done 
...
of the base field $\F_p$. The first row is the size of a randomly selected prime $p$ in bits. For a 
simpler implementation, and to exploit the low-level arithmetic of the GMP library \cite{gmp} in 
NTL, we have chosen random primes $p$ such that we alway have $\F_{p^2} = \F_p[T] / (T^2 + 1)$. 
Arithmetic in $\F_{p^2}$ for a general modulus $T^2 + cT + d$ is slightly slower.

For each prime, we have generated 10 random ordinary and 10 random supersingular curves 
using Sage \cite{stein2008sage}. Since most of the elliptic curves $E/\F_q$ are ordinary, the 
generation of random ordinary curves is done by simply choosing random coefficients $a, b$ for the 
Weierstrass equation. Generating supersingular curves can be efficiently done using the Complex 
Multiplication method in \cite{broker2009}. As pointed out in \cite{sutherland2012}, one can start 
from a curves constructed by the method of \cite{broker2009} and take a random walk in the 
$2$-isogeny graph to get a random supersingular curve.
...
\begin{table}[H]
	...
	\begin{tabular}{ccccc}
		...
		\bfseries Prime size & Alg. \ref{alg:ss-strong} & Sage & Alg. \ref{alg:ss-strong} & Sage \\
		\midrule[0.7pt]
		32 & 0.0001 & 0.0096 & 0.020 & \\
		64 & 0.0002 & 0.0148 & 0.036 & \\
		128 & 0.0002 & 0.0359 & 0.086 & \\
		256 & 0.0005 & 0.1289 & 0.186 & \\
		385 & 0.0009 & 0.7071 & 0.336 & \\
		512 & 0.0017 & 1.0880 & 0.549 & \\
		640 & 0.0030 & 1.9945 & 0.773 & \\
		768 & 0.0051 & 5.4015 & 1.266 & \\
		896 & 0.0131 & 4.9720 & 1.791 & \\
		1024 & 0.0202 & 8.5884 & 2.133 & \\
		\midrule[0.7pt]
	\end{tabular}
	\label{table:exper}
