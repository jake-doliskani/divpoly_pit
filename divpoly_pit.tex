...
%In fact, experimental use-cases are better estimated by the following stronger lower bound 
...
%conjecture for general varieties. In our context, this implies that if $P \in E(\F_{q^n})$ does 
%not 
...
Explicit parameters can be computed as follows. Let $r > 0$ be a prime such that $p^2$ is a 
generator of $(\Z/r\Z)^*$. Then $p^2$ has order $d = r - 1$, and $d^{2 - \epsilon} > r$ for a wide 
range of values of $0 < \epsilon < 1$. Now the $r$-th cyclotomic polynomial $\Phi_r(T)$ is 
irreducible over $\F_q$ so that $K = \F_q[T]/\Phi_r(T)$ is a field. Let $t$ be the image of $T$ in 
$K$. A point $P \in E$ with first coordinate $t$ lies in $E(F)$ where $[F : K] \le 2$. If $P$ 
satisfies (\ref{equ:ss-strong}) then $\ang{P}$ is invariant under the action of $\pi$. Then, by the 
above, the order of $P$ is at least $\binom{\abs{J}}{T}$. We need choose a suitable $r$ and 
optimize for the parameters $\epsilon, \eta$ so that
\begin{equation}
	\label{equ:binom}
	\binom{\abs{J}}{T} \gg \binom{d^{2 - \epsilon / 3} / r - r^{\epsilon / 3}}{d^\eta} \approx 
	\binom{r^{1 - \epsilon / 3} - r^{\epsilon / 3}}{r^\eta} \ge 2p + 2.
\end{equation}
Then $P$ will have order large enough to imply that $\psi_p = 1$. This gives Algorithm 
\ref{alg:ss-strong} for testing the PIT (\ref{equ:ss-strong}).
