...
$\F_p$. The first row is the size of a randomly selected prime $p$ in bits. For a simpler 
implementation, and to exploit the low-level arithmetic of the GMP library \cite{gmp} in NTL, we 
have chosen random primes $p$ such that we alway have $\F_{p^2} = \F_p[T] / (T^2 + 1)$. Arithmetic 
in $\F_{p^2}$ for a general modulus $T^2 + cT + d$ would be slightly slower.
...
\begin{table}[H]
	...
	\begin{tabular}{ccccc}
		...
		\bfseries Prime size & Alg. \ref{alg:ss-strong} & IsoGr & Alg. \ref{alg:ss-strong} & IsoGr 
		\\
		\midrule[0.7pt]
